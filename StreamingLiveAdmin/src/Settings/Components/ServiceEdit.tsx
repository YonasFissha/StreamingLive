...
import { ServiceInterface, ApiHelper, InputBox, FormatHelper } from './';
...
export const ServiceEdit: React.FC<Props> = (props) => {
    ...
    const handleDelete = () => {
        if (window.confirm('Are you sure you wish to delete this service?')) {
            ApiHelper.apiDelete('/services/' + currentService.id).then(() => { setCurrentService(null); props.updatedFunction(); });
        }
    }
    ...
    const handleSave = () => {
        setVideoUrl();
        ApiHelper.apiPost('/services', [currentService]).then(props.updatedFunction);
    }
            ...
            <Row>
                ...
                <Col>
                    <FormGroup>
                        ...
                        <select className="form-control" name="recurs" value={new Boolean(currentService?.recurring).toString()} onChange={handleChange} >
