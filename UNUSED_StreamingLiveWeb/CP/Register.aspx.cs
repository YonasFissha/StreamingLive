...
namespace StreamingLiveWeb.CP
{
    public partial class Register : System.Web.UI.Page
    {
        ...
        {
            /*
            string[] errors = ValidateCampus();
            if (errors.Length == 0)
            {
                StreamingLiveLib.Site s = new StreamingLiveLib.Site() { KeyName = KeyNameText.Text.ToLower().Trim() };
                s.Save();

                StreamingLiveLib.User u = new StreamingLiveLib.User() { Email = EmailText.Text.ToLower().Trim(), Password = StreamingLiveLib.User.HashPassword(PasswordText.Text.Trim()) };
                u.ResetGuid = Guid.NewGuid().ToString();
                u.Save();

                StreamingLiveLib.Role r = new StreamingLiveLib.Role() { Name = "owner", SiteId = s.Id.Value, UserId = u.Id.Value };
                r.Save();
